...
# https://de.wikipedia.org/wiki/Effektstärke
...
(etaSq <- SSb / (SSb + SSw)) # Effektstärke
library(DescTools)                     # for EtaSq()
EtaSq(aovCRp, type=1) 

(omegaSq <- dfSSb * (MSb-MSw) / (SSb + SSw + MSw)) # Populationseffektschäter
(f <- sqrt(etaSq / (1-etaSq))) # Effektstärke
